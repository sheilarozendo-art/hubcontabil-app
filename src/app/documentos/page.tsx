...
function DocumentosPage({ onNav, menuOpen, onOpenMenu, onCloseMenu }: {
  onNav: (p: string) => void
  menuOpen: boolean
  onOpenMenu: () => void
  onCloseMenu: () => void
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#F5F7FB', overflowX: 'hidden' }}>
      <TributosNav active="docs" mobileOpen={menuOpen} onClose={onCloseMenu} onNav={onNav} />

      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflowX: 'hidden' }}>
        <Topbar
          onOpenMenu={onOpenMenu}
          title="Cofre de Documentos & GED"
          subtitle="Armazenamento seguro em nuvem com conformidade LGPD e ICP-Brasil"
        />
  ...
  return (
    ...
    >
      ...
      >
        <Topbar
          onOpenMenu={onOpenMenu}
          title="Tributos & Guias Fiscais"
          subtitle="Gestão de guias, apurações e comprovantes"
        />
...
// Sidebar variant that accepts onNav for page switching
function TributosNav({
  active,
  mobileOpen,
  onClose,
  onNav,
...
export default function App() {
  ...
  if (page === "guias") {
    return <>{sharedPageStyle}<TributosPage onNav={setPage} menuOpen={menuOpen} onOpenMenu={() => setMenuOpen(true)} onCloseMenu={() => setMenuOpen(false)} /></>
  }

  if (page === "docs") {
    return <>{sharedPageStyle}<DocumentosPage onNav={setPage} menuOpen={menuOpen} onOpenMenu={() => setMenuOpen(true)} onCloseMenu={() => setMenuOpen(false)} /></>
  }
