...
export default function Topbar({ 
  ...
  return (
    <header className="h-16 lg:h-[76px] bg-white border-b border-[#E2E8F0] flex items-center justify-between px-4 sm:px-6 lg:px-8 shrink-0 font-sans sticky top-0 z-20">
      
      {/* Lado Esquerdo: Risquinhos Hambúrguer no Celular + Logo Oficial / Título */}
      <div className="flex items-center gap-3.5">
        {/* BOTÃO COM OS 3 RISQUINHOS (SÓ NO CELULAR/TABLET) */}
        <button
          onClick={onOpenMenu}
          className="btn-menu-hamburguer p-2 -ml-1 text-[#030303] hover:bg-slate-100 rounded-lg border-none bg-transparent cursor-pointer items-center justify-center transition"
          aria-label="Abrir Menu Lateral"
        >
          <Menu size={22} />
        </button>

        {/* LOGO OFICIAL NO CABEÇALHO DO CELULAR */}
        <div className="logo-mobile-topbar">
          <Logo tamanho="sm" comTexto={false} />
        </div>

        <div>
          <h2 className="text-sm sm:text-lg font-extrabold text-[#030303] tracking-tight leading-tight">
            {title}
          </h2>
      ...
      {/* Lado Direito: Busca + Status + Sino + Nova Solicitação */}
